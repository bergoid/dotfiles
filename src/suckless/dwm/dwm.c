...
load_config_from_env(void)
{
    ...
    fonts[0] = font_name;
    dmenucmd[4] = font_name;

    // Foreground color
    static char fg_color[8];
    copy_env_var(fg_color, "LOCAL_DWM_FG_COLOR", DEFAULT_FG_COLOR, sizeof(fg_color));
    colors[SchemeNorm][0] = fg_color;
    colors[SchemeSel][1] = fg_color;
    dmenucmd[8] = fg_color;
    dmenucmd[10] = fg_color;

    // Background color
    static char bg_color[8];
    copy_env_var(bg_color, "LOCAL_DWM_BG_COLOR", DEFAULT_BG_COLOR, sizeof(bg_color));
    root_bg_color = bg_color;
    colors[SchemeNorm][1] = bg_color;
    colors[SchemeNorm][2] = bg_color;
    colors[SchemeSel][0] = bg_color;
    colors[SchemeSel][2] = bg_color;
    dmenucmd[6] = bg_color;
    dmenucmd[12] = bg_color;
...
void hex2xcolor(const char* hex, XColor* pcolor)
{
    static const float scale_factor = 65535./255.;
    ...
    {
        die("Error parsing color hex string");
    }

    pcolor->red = (unsigned short)(scale_factor*red + .5);
    pcolor->green = (unsigned short)(scale_factor*green + .5);
    pcolor->blue = (unsigned short)(scale_factor*blue + .5);
...
setup(void)
{
    ...
    /* Load customization from environment vars */
    ...
    /* set root window bg color  */
    XColor col;
    hex2xcolor(root_bg_color, &col);
